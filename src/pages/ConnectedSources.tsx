...
const TeamsIcon = ({ className = "" }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 2228.833 2073.333" className={className}>
...
// Connection Status Component
  ...
  return (
    <div className="flex flex-col min-h-[310px]">
      ...
      {/* Action Buttons */}
      <div className="space-y-2 mb-3">
          {/* Sync Documents Button for Google Drive, Notion, Slack, and Teams */}
          {(connection.source_type === 'google_drive' || connection.source_type === 'notion' || connection.source_type === 'slack' || connection.source_type === 'teams') && onSyncDocuments && (
        ...
        case 'teams':
          return {
            ...
            wontAccess: [
              ...
              'Cannot modify teams or channels',
...
const ConnectedSources = () => {
  ...
  const handleConnect = async (sourceId: string) => {
    ...
    try {
      ...
      if (sourceId === 'googleDrive') {
      ...
      } else if (sourceId === 'teams') {
        console.log('🎯 Starting Teams OAuth flow...');
        const clientId = getEnvVar('VITE_MICROSOFT_CLIENT_ID');
        const tenantId = getEnvVar('VITE_MICROSOFT_TENANT_ID');
        
        console.log('📊 Teams OAuth credentials check:');
        ...
        if (!clientId || !tenantId) {
          ...
          console.error('   Missing:', { clientId: !clientId, tenantId: !tenantId });
          return;
        }
        ...
        console.log('📊 State data:', { userId: user?.id, timestamp: Date.now(), source: 'teams' });
        ...
        console.log('   Base URL:', `https://login.microsoftonline.com/${tenantId}/oauth2/v2.0/authorize`);
        console.log('   Client ID:', clientId);
        console.log('   Redirect URI:', `${apiUrl}/api/auth/teams/callback`);
        console.log('   Scope:', 'https://graph.microsoft.com/.default offline_access');
        console.log('   State:', state);
        console.log('🔗 Full Teams OAuth URL:', teamsAuthUrl.toString());
        
        window.location.href = teamsAuthUrl.toString();
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <div className="flex-1">
          ...
          onRefresh={() => {
            const sourceTypeMap: Record<string, string> = { 'googleDrive': 'google_drive', 'notion': 'notion', 'slack': 'slack', 'teams': 'teams' };
            const dbSourceType = sourceTypeMap[source.id] || source.id;
          ...
          onSyncDocuments={(source.id === 'googleDrive' || source.id === 'notion' || source.id === 'slack' || source.id === 'teams') ? () => {
            const sourceTypeMap: Record<string, string> = { 'googleDrive': 'google_drive', 'notion': 'notion', 'slack': 'slack', 'teams': 'teams' };
            const dbSourceType = sourceTypeMap[source.id] || source.id;
