...
export default function ActionPlanViewer({ meetingId }: { meetingId: string }) {
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://localhost:8085'}/api/v1/meetings/${meetingId}/plan?user_id=${userId}`, {
        method: 'POST'
      })
  ...
  const executeAction = async (action: AgentAction, index: number) => {
    setExecuting(index)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://localhost:8085'}/api/v1/actions/execute`, {
        method: 'POST',
