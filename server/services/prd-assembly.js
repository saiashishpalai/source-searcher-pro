...
Goal: Deliver a 70-90% complete PRD with Requirements detailed enough that engineering can build without asking additional questions.`;
    ...
    try {
      ...
      // Format 1: { sections: [...], summary: {...} } (expected)
      // Format 2: { "1. Objective": "...", "2. Background": "..." } (actual)
      ...
      if (Array.isArray(parsed.sections)) {
        ...
        // Model returned flat object with numbered keys - convert to sections array
        console.log('⚠️  Model returned flat format, converting to sections array...');
        const sectionKeys = Object.keys(parsed).filter(k => k.match(/^\d+\./));

        const extractBulletList = (text) => {
          if (!text) return [];
          return text
            .split('\n')
            .map(line => line.trim())
        ...
        generatedSections = sectionKeys.map(key => {
          const content = parsed[key] || '';
          const match = key.match(/^(\d+)\.\s*(.+)$/);
          const number = match ? parseInt(match[1], 10) : 0;
          const title = match ? match[2] : key;

          let confidence = 90;
          let needsValidation = [];
          let requiresInput = [];
          let confidenceRationale = 'Generated by AI from user inputs and workspace context';
          let missingContext = '';

          // Detect low confidence block
          const lowMatch = content.match(/\[EMPTY\s*-\s*LOW CONFIDENCE:\s*(\d+)%\]/i);
          if (lowMatch) {
            confidence = parseInt(lowMatch[1], 10);
            const clarificationMatch = content.match(/This section requires clarification:\s*([\s\S]*?)(?:\n\n\[[A-Z]|$)/i);
            if (clarificationMatch) {
              requiresInput = extractBulletList(clarificationMatch[1]);
            }
          }

          // Detect medium confidence validation block
          const validationMatch = content.match(/\[CONFIDENCE:\s*(\d+)%\s*-\s*Please validate:\s*([\s\S]*?)\]/i);
          if (validationMatch) {
            confidence = parseInt(validationMatch[1], 10);
            needsValidation = extractBulletList(validationMatch[2]);
          }

          // Extract context / rationale if present
          const contextMatch = content.match(/\[Context:\s*([^\]]+)\]/i);
          if (contextMatch) {
            confidenceRationale = contextMatch[1].trim();
            if (lowMatch) {
              missingContext = confidenceRationale;
            }
          }
      ...
      if (!summary || Object.keys(summary).length === 0) {
        const highCount = generatedSections.filter(s => (s.confidence_percent ?? 0) >= 70).length;
        const mediumCount = generatedSections.filter(s => (s.confidence_percent ?? 0) >= 50 && (s.confidence_percent ?? 0) < 70).length;
        const lowCount = generatedSections.filter(s => (s.confidence_percent ?? 0) < 50).length;
        const overallConfidence = generatedSections.length
          ? Math.round(generatedSections.reduce((sum, s) => sum + (s.confidence_percent ?? 0), 0) / generatedSections.length)
          : 0;
        summary = {
